...
try:
    with open(dir_path+sfile,'r') as sref:
        refs = sref.read()
        params = refs.split(params_end)[0]
except IOError:
    try:
        with gzip.open(dir_path+StarName+'.ws.gz','r') as ziprefs:
            cleaned = True
            refs = ziprefs.read()
        ...
        print dir_path+sfile,'file not found'
        exit(0)
...
if dir_path:
    bfile = dir_path+bfile
    if cleaned:
        blist = sorted(glob.glob(dir_path+'*.b[0-9]*'))
        blist_num = []
        ...
        if modanf-closestb != 0:
            ...
            print "you'll have to start with model",models[closestb],'or',models[nextb],'\n'
            exit(0)
        else:
            if bfile not in blist:
                if bfile+'.gz' in blist:
                    ...
                    print bfile.rsplit('/',1)[1],'not found in',dir_path
    os.system('cp '+bfile+' .')
    if zipped:
        os.system('gunzip '+bfile[bfile.rfind('/')+1:])
    os.system('cp '+dir_path+'net* .')

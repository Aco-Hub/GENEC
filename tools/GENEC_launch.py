...
parser = argparse.ArgumentParser(
    ...
    usage=(
        ...
        f'\n'
        f'You can specify a phase (or model) to stop at with option -p #phase (-m #model)\n'
        f'\n'
...
with open('computation.log', 'a', encoding='ENCODING') as logfile:
    ...
    if initial_file == '':
        logfile.write(f'starting model is {str(nwseq)}, phase: {str(phase)}\n')
    if calc_dir != '':
        logfile.write(f'Computation performed in directory: {calc_dir}\n')
    logfile.write(f'Program used: {ExecGenec}\n')
    if initial_file != '':
...
if initial_file != '':
    if f'{StarName}.input' in os.listdir('.'):
        answer = input(
    ...
    if answer.lower() in 'yes':
        ...
        if runstat != 'running':
            ...
            if Zipping:
                CommandZip = (
                    f'gzip -f {StarName}.[l,v,x,y]0000001 {StarName}.b00000 '
                    f'{StarName}.b00001 {StarName}_StrucData_0000001.dat'
                )
...
while True:
    ...
    if os.path.isfile(f'{StarName}.b{modanf:05d}.gz'):
        needed_for_calc = needed_for_calc+[f'{StarName}.b{modanf:05d}.gz']
    else:
        needed_for_calc = needed_for_calc+[f'{StarName}.b{modanf:05d}']
    try:
    ...
    if Zipping:
        CommandZip = (
            f'gzip -f '
            f'{StarName}.[l,v,x,y]{nwseq:07d} '
            f'{StarName}.b{modanf:05d} '
            f'{StarName}.b{modanf+1:05d} '
    ...
    if calc_dir != '' and time_to_transfer:
        ...
        try:
            result_files = result_files+[f'.PlotData_{StarName}']
        except:
            pass
    ...
    try:
        ...
        if (
            "up" in LoopMode
            and (initial_loop[0] >= loop_min or initial_loop[1] >= loop_min)
        ):
            restart_loop = True
        elif (
            "down" in LoopMode
            and (initial_loop[0] <= loop_max or initial_loop[1] <= loop_max)
        ):
            restart_loop = True
        else:
    ...
    if runstat != 'running':
        if runstat != '':
            if 'ZAMS reached' in runstat:
                ...
                print(f'ZAMS reached: NWSEQ= {nwseq}, MODANF= {modanf}, NZMOD= {nzmod}')
                if Zipping:
                    if os.path.isfile(f'{StarName}.b{modanf:05d}.gz'):
                        os.system(
                            f'gunzip {StarName}.b{modanf:05d}.gz'
                        )
                os.system(CommandLaunch)
                if Zipping:
                    CommandZip = (

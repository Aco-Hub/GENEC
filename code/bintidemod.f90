...
!----------------------------------------------------------------------
  ...
  real(kindreal):: spiper,regra,qr1,qr2,rltid,fact1,fact2,fact3,fact4,e2,tsyn,roint,&
       rroche,radcon,e21,tsyn1,fact5,s22,dltido,fas1,fas2,&
       difsav,trot
!----------------------------------------------------------------------
...
!     especially the radius of the convective core (see Yoon et al. 2010)
  ...
  if (abs(period/day-spiper)>2.0d-2)then
  ...
  write(80,'(2X,I4,21(2X,1P,D16.9))') nwmd,alter,dzeit/year,period/day,spiper,omegi(1),romini,tsyn/year,&
         tsyn1/year,rstar/Rsol,vsuminenv,rroche/Rsol,rstar/rroche,dltid,rltid,fact1,fact2,fact3,fact4,e2,&
         trot/year,dltido
...
!----------------------------------------------------------------------
  use caramodele, only: gms,gls,teff,dm_lost,nwmd
  use timestep,only: alter,dzeit

  implicit none

  real(kindreal),intent(in)::  dLtid
  real(kindreal):: dab,qr1,qr2,orbang,orstwi,rorstw,fact1,fact2,fact3,rltid
!----------------------------------------------------------------------
  qr1=binm2/(binm2+gms)
  qr2=1.0d0/qr1
  orbang=(gms*Msol*(qr1*ab)**2.d0+binm2*Msol*(qr2*ab)**2.d0)*romorb
  orstwi=dm_lost*Msol*(qr1*ab)**2.d0*romorb
  rorstw=orstwi/dzeit
  fact1=2.d0*(dltid+orstwi)/orbang
  rltid=dLtid/(dzeit/year)
  fact2=-2.d0*dm_lost/gms
  fact3=dm_lost/(binm2+gms)
  dab=ab*(fact1+fact2+fact3)
  ab=ab+dab
  ...
  write(3,*) 'alter=',alter,'dltid=',dltid,'orstwi=',orstwi,'romini=',romini,'rmw=',dm_lost/(dzeit/year),&
       'fact1=',fact1,'fact2=',fact2,'fact3=',fact3,'dab=', dab/ab,'binm2=',binm2,'rstar/rsun=',rstar/Rsol,&
       'ab*qr1/rsun=',ab*qr1/Rsol
  write(81,'(2X,I4,15(2X,1P,D16.9))') nwmd,alter,period/day,dzeit/year,romini,gms,binm2,dltid,orstwi,&
       rorstw,orbang,dm_lost/(dzeit/year),fact1,fact2,fact3,dab/ab

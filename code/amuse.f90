module amuse_helpers
    use inputparam, only: &
            starname, nwseq, modanf, nzmod, &
            irot, isol, imagn, ialflu, ianiso, ipop3, ibasnet, phase, var_rates, bintide, binM2, periodini, const_per, &
            zinit, zsol, z, iopac, ikappa, &
            idiff, iadvec, istati, icoeff, fenerg, richac, igamma, frein, K_Kawaler, Omega_saturation, &
            rapcrilim, vwant, xfom, omega, xdial, idialo, idialu, Add_Flux, diff_only, B_initial, add_diff, &
            imloss, fmlos, lowRSGMdot, ifitm, fitmi, fitm, deltal, deltat, nndr, &
            iledou, idifcon, elph, my, iover, dovhp, iunder, dunder, &
            gkorm, alph, agdr, faktor, dgrp, dgrl, dgry, dgrc, dgro, dgr20, nbchx, nrband, &
            islow, xcn, icncst, tauH_fit, &
            plot, refresh, iauto, iprn, iout, itmin, xyfiles, idebug, itests, verbose, stop_deg, &
            Write_namelist
    use genec, only: &
            lec_geo, ichem, CorrOmega, xLtotbeg, dlelex, dlelexprev, &
            Flux_remaining, dlelexsave, Btotal_EndAdvect, timestep_control, elemneg, ielemneg, &
            CorrZero, supraEdd, rhocprev, Tcprev, Iteration48, IterTriangle, TriangleIteration, &
            suminenv, fffff, iprezams, veryFirst, &
            tzero, idern, id1, id2, dm_lost, agdp, agds, agdt, dgrp, um, restart, i, zabelx, xnetalu, &
            isugi, nsugi, mmax, inum, modell, nzmodini, nfseq, nwmd, &
            IniStruc, xmini, summas, zams_radius, period, day, omegi, alter, dzeit, firstmods, &
            exphi, q, &
            xf19, xne21, xna23, xal26, xal27, xsi28, xneut, xprot, xc14, xf18, xbid1, xsi28, &
            bibib, xmg26, xmg25, xmg24, xne22, xo18, xo17, xo16, xn15, xn14, xc13, xc12, y3, y, x, &
            xne20, nbelx, ii, abels, xbid, m, abelx, &
            NPcoucheEff, npondcouche, ab, p, t, r, s, vp, vt, vr, vs, izurrs, &
            q, vsuminenv, vvsuminenv, npondcoucheAdv, vomegi, xltod, xini, &
            gms, gls, teff, glsv, teffv
    use LayersShift, only: fitmshift
    use geomod, only: initgeo
    use omegamod, only: momevo
    use energy, only: netinit
    use WriteSaveClose, only: write4
    ...
    real(8):: dzeitj,dzeitv,n,Lstar,xteff,rstar,alpha,&
    ...
    contains
        subroutine makeini
            ...
            xo17 = longueur * xx(8)
            xo18 = longueur * xx(9)
            xne20 = longueur * xx(10)
            xne22 = longueur * xx(11)
            xmg24 = longueur * xx(12)
            xmg25 = longueur * xx(13)
            xmg26 = longueur * xx(14)
            do i=1,15

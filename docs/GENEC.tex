...
%:   PhysicsParams
  \item[\fbox{PhysicsParams}] ~\\ contains the choices in the included physics and the evolutive phase. The parameters in this list are:
    \begin{description}
      ...
      \item[BINTIDE:] tidal interaction in binaries if set to \texttt{T}. Uses the following parameters:
      \item[\hspace*{1cm}BINM2:] the mass of the companion;
      \item[\hspace*{1cm}PERIODINI:] the initial period of the binary;
      \item[\hspace*{1cm}CONST\_PER:] a switch to keep a constant period if set to \texttt{T} (default) or to compute the period evolution otherwise.
    \end{description}
    Facultative parameters are \texttt{ieos}, \texttt \texttt{imagn}, \texttt{ianiso}, \texttt{ipop3}, \texttt{ibasnet} (default  \texttt{0}), \texttt{var\_rates} and \texttt{bintide} (default \texttt{F}).
...
%:   CompositionParams
  \item[\fbox{CompositionParams}] ~\\ contains the chemical characteristics of the model. The parameters in this list are:
    ...
    \end{description}
    \texttt{zsol=0.014}, \texttt{ikappa=5}, and  \texttt{iopac=3} are facultative.
...
https://ui.adsabs.harvard.edu/abs/2004A&A...425..243M}{Mathis+ 2004} \\
 ...
 $D_\text{tot}$ \href{https://ui.adsabs.harvard.edu/abs/2013A&A...553A...1M}{Maeder 2013} & {\ttfamily\textbf{31}} & {\ttfamily\textbf{32}} & {\ttfamily\textbf{33}} \\
    ...
    \end{description}
    \texttt{istati=0}, \texttt{fenerg=1.0}, \texttt{richac=1.0}, \texttt{gamma=0}, \texttt{A\_M03=0.0}, \texttt{n\_M03=0}, \texttt{frein=0.0}, \texttt{K\_Kawaler=0.0},\\ \texttt{Omega\_saturation=14.0}, \texttt{vwant=0.0}, \texttt{xfom=1.0}, \texttt{Add\_Flux=T}, \texttt{diff\_only=F}, \texttt{B\_initial=0.0}, \texttt{add\_diff=0.0}, \texttt{n\_mag=1}, \texttt{alpha\_F=6.0}, \texttt{nsmooth=5}, \texttt{qminsmooth=T}, and \texttt{dcirch\_inclusion=F} are facultative. Leaving \texttt{A\_M03} and \texttt{n\_M03} as default gives the old value of $A=0.002$ in the expression for the {\ddh} of Maeder 2003.

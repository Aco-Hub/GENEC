...
!         new file name + err option
  ...
  open(io_opacity, file=opacfile,iostat=error_readco)
  if (verbose) then
...
! old goto 1234
  if (error_readco /= 0) then
    ...
    rewind(io_runfile)
    write(io_runfile,*) 'File ',trim(opacfile),' not found !!'
    stop
  endif
...
! check first value of t6
  ...
  if (ntemp > ip) then
    rewind(io_runfile)
    write(io_runfile,*) nwmd,'STOP opaltab: require parameter ip of at least ',ntemp
    ...
    if (t6 > tmin) then
      if (t6 > 0.004d0)then
        if (x_kap >= 0.35d0)then
          ...
          if (z_kap > 0.5d0) then
            write(*,*)'on est trop en dehors des tables d''opacite'
            write(*,'(1x,"t6 =",f9.3," r = rho/t6^3 =",f8.3," ln(rho) =",f9.3,"ln(T) =",f9.3,/," X =",f9.3,"Y =",f9.3,&
                   & " z_kap =",f9.3)') t6,r,rh,t,x_kap,y_kap,z_kap

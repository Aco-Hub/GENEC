...
! Second condition because if (ifin-ideb-2) < 2*WinSize+1 the smoothing is not performed (see smallfunc)
     if ((ifin-ideb-2) < 4*WinSize+1 .and. (ifin-ideb-2) >= 2*WinSize+1) then
       write(io_logs,*) 'mu profile smoothed on a relatively small zone: window = ',2*WinSize+1,' zone to fit : ',&
...
! on accepte un ecart de 0.005*mu(surface) et un chi^2 < 1.d-5:
    ...
    write(io_logs,'(a,1p,2(1x,d23.16))')'GRAPMUI - chi^2,chi^2 accepted:',chimu2,1.d-4*amumax
    write(io_logs,'(a,1p,2(1x,d23.16))')'          distmax,chimutol:',distmax,chimutol
